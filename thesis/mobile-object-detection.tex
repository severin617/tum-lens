...
\begin{figure}[H]
	...
	\begin{tikzpicture}[node distance=4cm]
		\node[box] (ml) {\textbf{Classification of Machine Learning}};
		\node[box, above left of = ml] (supervised) {
			\begin{minipage}{5cm}
				\centering\small\textbf{Supervised Learning}
				\begin{itemize}
					\item \small\texttt{Regression}
					\item \small\texttt{Classification}
				\end{itemize}
			\end{minipage}
		};
		\node[box, above right of = ml] (unsupervised) {
			\begin{minipage}{5cm}
				\centering\small\textbf{Unsupervised Learning}
				\begin{itemize}
					\item \small\texttt{Clustering}
					\item \small\texttt{Association}
				\end{itemize}
			\end{minipage}
		};
		\node[box,  below right of = ml] (semisupervised) {
			\begin{minipage}{5cm}
				\centering\small\textbf{Semi-supervised Learning}
				\begin{itemize}
					\item \small\texttt{Classification}
					\item \small\texttt{Clustering}
				\end{itemize}
		...
		\node[box, below left of = ml] (reinforcement) {
			\begin{minipage}{5cm}
				\centering\small\textbf{Reinformcement Learning}
				\begin{itemize}
					\item \small\texttt{Classification}
					\item \small\texttt{Control}
				\end{itemize}
			\end{minipage}
		};
...
%Nowadays, there are two common types of detectors: two-stage and single-stage detectors.
...
\begin{table}[h]
	...
	\end{tabularx}
	\caption[Java Files in TUM-Lens v1.0 And Change in Lines of Code After Their Conversion to Kotlin]{This table presents the results from the command line prompt in \autoref{code:cloc}. Packages are shown as a prefix to the file name to resemble the original project structure of TUM-Lens v1.0 and file extension are omitted.}
	\label{tab:cloc}
\end{table}
...
\begin{lstlisting}[style=standard, language=bash, label=code:adb, caption={The \gls{adb} offer a convenient way to retreive trace files saved on an Android device. This command transfers the system trace from our test device to the development machine it is attached to via an usb cable.}]
