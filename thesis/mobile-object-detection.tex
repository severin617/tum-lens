...
There's a wide range of possible applications centred around the idea of on-device machine learning. We've already touched on the example of DeepType in \autoref{sec:privacy} and protecting the sphere of privacy will definitively be major driver for the development of apps utilising offline \gls{ml}.

Some company like Apple and Snap\footnote{Snap Inc. (\url{https://www.snapchat.com}) is the developer of the mobile app Snapchat.} are leading by example - at least in specific areas of their business practices. Apple's Face ID is a well-known security feature of Apple's smartphone division. It relies purely on on-device machine learning to pick up all the important details that make a face unique thereby successfully securing the device without transferring sensitive user data to the cloud. Snapchat\footnote{Snapchat is a mobile app known for its various and peculiar filters that its users can apply to manipulate the images captured by the smartphone camera.} uses on-device machine learning to power its manifold filters so that user data only gets transmitted once the user actively shares a photo or video via the app.

Since smartphones are just a subcategory of mobile devices, different hardware opens the door for new types of applications. Health care is one industry that can greatly benefit from on-device machine learning as health data is among the data categories that are most worthy of protection. Today, there're already lots of different devices that capture health data like smart watches, portable ecg\footnote{text} devices,    can happen on different hardware. In the domain of public security, CCTV in public places is a

A second important factor 

  to latencyDie Anwendungsbereiche von Computer Vision sind zahlreich. Zu den regelmäßigen Aufgaben im Bereich 
