...
While the baseline \gls{ann} described in section \ref{ssection:ann} only connects adjacent layers, \glspl{resnet} are a special subclass of \gls{ann} that introduces \textit{shortcut connections}~\cite{backboneResNet}. Shortcut connection are edges in the network that skip one or more layers.
...
For a start, Android Studio could not infer all types automatically an being strongly typed it was necessary to restructure the code so the compiler could safely cast a variable to the appropriate type (called \textit{Smart Cast} in Kotlin). Usually this fell under one of two cases. In the first case of non-critical functionality, using Kotlin's safe call operator \code{?.} to access properties that might be null was sufficient. When it came to core functions, an additional safety check was the better option so that the compiler could be certain that a mutable property had not accidentally become null before being accessed and we could also react accordingly if the safety check failed. \\
The automatic conversion that actually introduced a bug was the replacement of getters and setters. While it Java uses \code{getVariableName()} and \code{setVariableName(variableType newValue)} to retrieve or overwrite a property of a private class from outside, Kotlin does so 


\subsubsection{Measuring The Decrease of The Size of The Code Base} %TODO: Check if this should rather be moved to the Results chapter
...
\begin{lstlisting}[style=cmd, label=code:cloc, caption={Command line input to print analysis of lines of code before and after the Java to Kotlin conversion. The output is also saved as cloc.csv and can be found in lens-david/thesis/raw\_data.}]
	lens-david % npx cloc --by-file --git-diff-rel  --match-d='main' --include-lang=Java,Kotlin -csv f6a18e0f 031a26a7
\end{lstlisting}
