...
\noindent Three observations are important to understand put the performance measurements into context. Firstly, the device renders images at a fixed rate taking whatever input is currently available. 




The code which starts the \code{imageAvailable} section will only be entered if no image is currently being processed. Thus it only appears in between \code{recognizeImage} sections.
