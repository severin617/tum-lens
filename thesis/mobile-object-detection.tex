...
At the beginning of this chapter, we will have a look at the impact of changing the programming language from Java to Kotlin with regards to the number of lines of code. Next, the performance of the application is evaluated based on the goals that were set before we took on this thesis topic. Lastly, we showcase some real footage of the application indicating in which areas the object detection performs well and where it has its week points.

\section{Project Size After The Kotlin Migration}
...
One of the goals of this work is the expansion of TUM-Lens by integrating a real-time object detection feature running entirely on the device itself. With that in mind and judging by the updated version of the app we have produced, this goal is fully achieved. The updated version of the app can be installed on an Android device or run in the emulator by cloning our GitLab repository\footnote{\url{https://gitlab.lrz.de/dvrws/lens}}. We are also intending to release the updated version 2.0 to the Google PlayStore\footnote{\url{https://play.google.com/store/apps/details?id=com.maxjokel.lens}} to make it accessible for a broader audience.
...
\begin{lstlisting}[style=standard, language=sql, label=code:perfettoSql, caption={SQL command to query object detection related events tracked with Android's system tracing.}]
	...
	(name = 'imageAvailable' or name = 'recognizeImage')
\end{lstlisting}

\noindent The source code includes carefully placed \code{Trace.beginSection([SectionName])} and \code{Trace.endSection()} function calls so that these events are properly tracked. All timings were measured with the default configuration of using 4 CPU-cores for the object detection which can also be seen in \autoref{fig:perfetto}. The \textit{recognizeImage} slice is traced in the main detection thread and three further threads support it. While the 

\begin{figure}[H] % [H] for HERE
	\centering
	\includegraphics[width=\textwidth]{figures/perfetto_visualisation.png}
	\caption[Screenshot of visual systrace analysis with Perfetto UI]{The screenshot shows a systrace visualisation using the Perfetto UI. The horizontal axis shows the time starting at 0 seconds and the vertical axes lists important types of events. These events are sorted to reflect their chronology as close as possible.}
	\label{fig:perfetto}
\end{figure}

\noindent The code which starts the \code{imageAvailable} section will only be entered if no image is currently being processed. Thus it only appears in between \code{recognizeImage} sections.
...
The final tip is the expansion of the features of the app. With image classification and object detection we already cover to major disciplines in computer vision. In a follow-up project, instance segmentation could be implemented to further increase the capabilities of TUM-Lens. While this would add a entirely new feature, improvements to existing function are also possible. On the easier side of the enhancement spectrum are further customisation options for the user. While he can currently adjust the number of threads to perform the object detection tasks on, more options like changing the minimum confidence threshold a detection must have to be displayed are conceivable. Ranked among the more difficult advancements is the implementation of better tracking than the mechanism currently implemented and described in \autoref{ssec:detectionAndTracking}. \\

%TODO: Think about including customisation features such as changing the number of maximum returned object or the 
