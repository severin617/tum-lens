...
class Logger @JvmOverloads constructor(private val tag: String = DEFAULT_TAG,
    ...
    companion object {
        ...
        init {
            ...
            IGNORED_CLASS_NAMES?.run {
                add("dalvik.system.VMStack")
                add("java.lang.Thread")
                add(Logger::class.java.canonicalName)
            }
        }
    }
    ...
    private fun toMessage(format: String, vararg args: Any?): String {
        val suffix = if (args.isNotEmpty()) String.format(format, *args) else format
